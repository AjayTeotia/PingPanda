...
export const EmptyCategoryState = ({
    ...
    const hasEvent = data.data?.hasEvent;

    useEffect(() => {
        if (hasEvent) router.refresh()
    }, [hasEvent, router]);

    const codeSnippet = `await fetch('${process.env.NEXT_PUBLIC_URL}/api/events', {
        method: 'POST',
        headers: {
    ...
    return (
        <Card
            contentClassName="max-w-2xl w-full flex flex-col p-6"
